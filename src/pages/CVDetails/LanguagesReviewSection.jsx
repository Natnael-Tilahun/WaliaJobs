...
export const LanguagesReviewSection = () => {
  ...
  const isReferenceSectionCompleted = useSelector(
    (state) => state.cvCompletionInfo[FormSections.REFERENCES]
  );
  ...
  const handleUpdateLanguage = (id) => {
    navigate(`/CV-Details/1/Language-Section/${id}`);
  };
  ...
  const handleContinue = () => {
    ...
    isReferenceSectionCompleted
      ...
      : navigate('/CV-Details/1/References-Section/0');
  };
    ...
    >
      ...
      <div className="flex  justify-between gap-3 md:gap-5 lg:gap-10 flex-wrap w-full">
        <div className=" w-full h-auto flex flex-col gap-2 lg:gap-5">
          {languages &&
            languages.map(({ languageName, proficiencyLevel, id }, index) => (
              <div
          ...
          <NavLink
            to="/CV-Details/1/Language-Section/0"
